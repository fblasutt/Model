...
def compute_moments(M):
    ...
    try:
        ...
        # Construct regression of changes in consumption to changes in dw
...
def sensitivity_analysis(param_idx, n_points=11):
    """
    ...
    # Create grid for the parameter centered around baseline ±20%
    baseline_val = xc[param_idx]
    param_min = max(xl[param_idx], baseline_val - 0.2 * baseline_val)
    param_max = min(xu[param_idx], baseline_val + 0.2 * baseline_val)
...
if __name__ == '__main__':
    ...
    n_points = 3  # Number of grid points for each parameter
    ...
    for param_idx in range(len(param_names)):
        results = sensitivity_analysis(param_idx, n_points=n_points)
    ...
    for results in all_results:
        ...
        for moment_name in ['wife_empl', 'divorce_rate', 'expenditure_x_share']:
